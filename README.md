...
Some other app is grabbing F8 (Discord push-to-talk, OBS, a game overlay). Same for F6 not adding rows. Close the conflicting app, or edit `autoclicker.ahk` and change the `F6::` / `F8::` lines near the bottom (e.g. `F9::` / `F10::`).
...
legacy/            — the original Python/Tkinter version, kept as a fallback
```
...
- All state (point list, interval, jitter) is in memory only — closing the window resets everything.

## Legacy Python version

The original Python implementation is preserved under [`legacy/`](legacy/) for reference, or if you'd rather hack on it in Python. See `legacy/README.md` for its install/run instructions.
